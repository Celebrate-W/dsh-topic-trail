...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ...
      '.dtt-steps{border-top:1px solid rgba(255,255,255,0.07);padding:6px 8px 10px;display:flex;flex-direction:column;gap:4px;}',
        ...
        return h('div', { className: 'dtt-topic', key: topic.id },
          h('button', {
            ...
            onClick: function (e) {
              var next = expanded ? null : topic.id;
              setOpenTopic(next);
              if (next) scrollTopicToTop(e); // 展开时自动滚动到顶部，最大化可见空间
            },

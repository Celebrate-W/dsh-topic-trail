...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ...
      '  width:384px;max-height:76vh;display:flex;flex-direction:column;border-radius:16px;overflow:hidden;',
      ...
      '.dtt-head{display:flex;align-items:center;gap:8px;padding:10px 12px;cursor:move;user-select:none;',
      ...
      '.dtt-body{flex:1;min-height:0;overflow-y:auto;padding:8px;display:flex;flex-direction:column;gap:8px}',
      ...
      '.dtt-topic{border:1px solid rgba(255,255,255,0.09);border-radius:12px;overflow:hidden;min-height:52px;',
      ...
      '.dtt-topic-head{display:flex;align-items:flex-start;gap:9px;padding:11px 12px;cursor:pointer;text-align:left;width:100%;min-height:52px;',
      ...
      '.dtt-topic-title{font-weight:600;font-size:13px;word-break:break-all;line-height:1.45}',
      ...
      '.dtt-group-wrapper .dtt-topic{border:none;border-radius:0;background:transparent;margin-bottom:0;animation:none;display:block;height:auto;overflow:visible;min-height:52px}',
      ...
      '.dtt-group-wrapper.hover .dtt-group-children{display:block;padding:8px 12px 12px;animation:dtt-fade 0.3s ease both}',
      '.dtt-group-wrapper.hover .dtt-chevron{transform:rotate(90deg)}',
      '.dtt-group-wrapper .dtt-chevron{transition:transform 0.3s ease}',
      '.dtt-settings-foot{font-size:11px;color:#6b7280;padding:12px 2px 4px;line-height:1.6}',
      '@media (max-width:768px){.dtt-root{width:min(92vw,360px);max-height:64vh}}',
    ].join('');
    ...
    var KIND_LABEL = { user: '用户', assistant: 'AI', tool: '工具', reasoning: '思考' };
    ...
    function TopicTrailPanel() {
      ...
      useEffect(function () {
        ...
        async function refresh() {
          try {
            ...
            // 跟随对话切换（设置页开关控制）：
            //   dsh 当前会话变化时——此前在工作区级 → 切到该对话所属工作区；此前在对话级 → 切到该对话
            try {
              if (appConfig.followSession !== false && appCtx && typeof appCtx.inject === 'function') {
                appCtx.inject(['sessions'], function (scope) {
                  try {
                    ...
                    var curId = curSnap && curSnap.current !== undefined && curSnap.current !== null
                      ? String(curSnap.current) : null;
                    if (curId && curId !== lastFollowRef.current) {
                      lastFollowRef.current = curId;
                      if (selectedWsIdRef.current) {
                        var wsIdBySession = {};
                        Object.keys(wsData.byId || {}).forEach(function (wid) {
                          ...
                          wTopics.forEach(function (tp) {
                            (Array.isArray(tp.sources) ? tp.sources : []).forEach(function (src) {
                              if (src.sessionId) wsIdBySession[src.sessionId] = wid;
                            });
                          });
                        });
                        var wid2 = wsIdBySession[curId];
                        if (wid2 && wsData.byId[wid2]) setSelectedWsId(wid2);
                        else { setSelectedWsId(null); setSelectedId(curId); }
                      } else {
                        setSelectedId(curId);
                      }
                    }
      ...
      var doMerge = useCallback(function (topicIdA, topicIdB) {
        ...
        fetch('/plugins/topic-trail/merge', {
          ...
          .catch(function (e) { showNotice('合并失败：' + String(e && e.message || e), 'err'); })
      ...
      function toggleCollapsed() {
        ...
        if (willExpand) {
          // 展开：先记录大球位置，再展开面板，等渲染后获取标题栏位置，开始飞行
          var fabEl = document.querySelector('.dtt-fab');
          var fromX = 0, fromY = 0;
          if (fabEl) {
            var r = fabEl.getBoundingClientRect();
            fromX = r.left + r.width / 2 - 15;
            fromY = r.top + r.height / 2 - 15;
          }
          setCollapsed(false);
          try { localStorage.setItem(COLLAPSED_KEY, '0'); } catch { /* ignore */ }
          setOpenTopic(null); setNavOpen(false); setMenu(null);
          setTimeout(function () {
            var headEl = headEyeRef.current;
            if (headEl) {
              var hr = headEl.getBoundingClientRect();
              setFlying({
                x: fromX, y: fromY,
                targetX: hr.left + hr.width / 2 - 15,
                targetY: hr.top + hr.height / 2 - 15,
                phase: 'to-panel',
              });
              setTimeout(function () { setFlying(null); }, 320);
            }
          }, 20);
        } else {
          // 收起：从标题栏飞回原位置
          var headEl2 = headEyeRef.current;
          var fromX2 = 0, fromY2 = 0;
          if (headEl2) {
            var r2 = headEl2.getBoundingClientRect();
            fromX2 = r2.left + r2.width / 2 - 15;
            fromY2 = r2.top + r2.height / 2 - 15;
          }
          setFlying({
            x: fromX2, y: fromY2,
            targetX: xy.x !== undefined ? xy.x : window.innerWidth - 50,
            targetY: xy.y !== undefined ? xy.y : window.innerHeight - 50,
            phase: 'to-ball',
          });
          setTimeout(function () {
            setCollapsed(true);
      ...
      // 折叠 → 小悬浮球（可拖动，拖动后不触发展开；位置兜底 clamp，保证永不出屏）
      if (collapsed) {
        ...
        return h('button', {
          ...
          title: '工作线索（' + viewCount + ' 条）——拖动移动位置，点击展开',
          ...
          h('span', {
            className: 'dtt-fab-pupil' + (blinking ? ' blink' : ''),
      ...
      // 渲染位置兜底 clamp：即使 state/视口变化，面板也至少留 60px 可见，杜绝「消失」
      var dispX = xy.x, dispY = xy.y;
      if (dispX !== undefined) {
        var vw = window.innerWidth || 1280, vh = window.innerHeight || 800;
        var pw = Math.min(360, vw - 24);
        dispX = Math.max(-(pw - 60), Math.min(dispX, vw - 60));
        dispY = Math.max(0, Math.min(dispY, vh - 60));
      }
      var style = {
        left: dispX !== undefined ? dispX + 'px' : undefined,
        right: dispX !== undefined ? undefined : '20px',
        top: dispY !== undefined ? dispY + 'px' : undefined,
        bottom: dispY !== undefined ? undefined : '20px',
      };
      ...
      function openMenu(e, scope, label) {
        e.preventDefault();
        e.stopPropagation();
        // 先记录原始鼠标坐标，渲染后按实际尺寸测量再反向/clamp
        setMenu({ clientX: e.clientX, clientY: e.clientY, scope: scope, label: label });
        setMenuPos(null);
      }

      // 执行「开始总结线索」
      function doSummarize(scope) {
        var body = {};
        ...
        fetch('/plugins/topic-trail/summarize', {
          ...
          .then(function (d) {
            if (d.targets !== undefined) {
              showNotice('已开始总结 ' + d.targets + ' 个会话' + (d.imported ? '（新导入 ' + d.imported + '）' : ''), 'ok');
            } else {
              showNotice('总结完成', 'ok');
            }
            if (refreshRef.current) refreshRef.current();
          })
          .catch(function (e) { showNotice('总结失败：' + String(e && e.message || e), 'err'); });
      }
      ...
      function renderTopicCard(topic) {
        ...
        var steps = Array.isArray(topic.steps) ? topic.steps : [];
        ...
        return h('div', Object.assign({ className: cls, key: topic.id }, dragProps),
          h('button', {
            ...
            title: topic.locked ? (hover + '（已锁定，自动总结不会修改）') : (canDrag ? (hover + '（可拖动到另一条线索上合并）') : hover),
            onClick: function () { if (!dragTopicId) setOpenTopic(expanded ? null : topic.id); },
            onContextMenu: function (e) {
              openMenu(e, selectedId, '会话：「' + fmtTitle(selectedSession && selectedSession.title, 30) + '」');
            },
          },
          ...
          expanded
            ? h('div', { className: 'dtt-steps' },
                steps.length === 0
                  ? h('div', { className: 'dtt-empty' }, '暂无步骤')
                  : steps.map(function (s) { return renderStep(s, selectedId, steps.length); }),
              )
            : null,
        );
      }
      ...
      function renderGroupCard(groupId, title, subtitle, childTopics, hoverHint) {
        ...
        return h('div', {
          ...
          h('div', { className: 'dtt-topic dtt-group-card' },
            h('button', {
              className: 'dtt-topic-head',
              title: hoverHint + '（双击打开对话）',
              onDoubleClick: function () {
                if (firstSid) jumpToSession(firstSid, null, null);
              },
            },
              ...
              h('span', { className: 'dtt-topic-main' },
                ...
                h('div', { className: 'dtt-topic-reveal' },
                  ...
                  h('div', { className: 'dtt-topic-meta' },
                    ...
                    activeCount > 0 ? h('span', { className: 'dtt-step-running' }, activeCount + ' 进行中') : h('span', null, '已完成'),
                  ),
                ),
              ),
          ...
          h('div', { className: 'dtt-group-children' },
            childTopics.length === 0
              ? h('div', { className: 'dtt-empty' }, '暂无线索')
      ...
      function renderEye(size, count) {
        ...
        return [
          h('span', {
            className: 'dtt-fab-pupil' + (blinking ? ' blink' : ''),
            style: {
              transform: 'translate(' + (pupilOffset.x * pupilScale) + 'px,' + (pupilOffset.y * pupilScale) + 'px)',
      ...
      var head = h('div', { className: 'dtt-head', onPointerDown: onPointerDown },
        h('span', { className: 'dtt-title' },
          h('span', {
            className: 'dtt-head-eye' + (isBusy ? ' busy' : ''),
            ref: headEyeRef,
            style: flying ? { opacity: 0 } : null,
          },
            renderEye(30, viewCount),
          ),
          '工作线索',
        ),
        h('span', { className: 'dtt-nav' + (navOpen ? ' dtt-nav-open' : '') },
          h('button', {
            ...
            h('span', { className: 'dtt-nav-label' }, currentNav ? currentNav.label : '选择会话'),
        ...
        h('button', {
          className: 'dtt-btn',
          title: '刷新',
          onClick: function () { setRefreshTick(function (t) { return t + 1; }); },
        }, '↻'),
      ...
      if (error) {
        ...
        body = h('div', { className: 'dtt-import-box' },
          ...
          h('button', {
            ...
            disabled: importing,
            onClick: doImport,
          }, importing ? '正在导入…' : '导入此会话'),
          importError ? h('div', { className: 'dtt-import-err' }, '导入失败：' + importError) : null,
        );
      } else {
      ...
      return h('div', { className: 'dtt-root', style: style },
        ...
        flying ? h('button', {
          className: 'dtt-fab dtt-fab-flying' + (isBusy ? ' busy' : ''),
          style: {
            left: flying.x + 'px',
            top: flying.y + 'px',
            transform: 'translate(' + (flying.targetX - flying.x) + 'px,' + (flying.targetY - flying.y) + 'px)',
          },
        },
          h('span', {
            className: 'dtt-fab-pupil' + (blinking ? ' blink' : ''),
      ...
      function update(patch) {
        ...
        fetch('/plugins/topic-trail/config', {
          ...
          .then(function (d) {
            if (d && d.config) {
              setCfg(d.config);
              if (typeof d.config.followSession === 'boolean') appConfig.followSession = d.config.followSession;
            }
          })
      ...
      return h('div', { className: 'dtt-settings' },
        h('div', { className: 'dtt-settings-row' },
          h('div', { className: 'dtt-settings-text' },
            h('div', { className: 'dtt-settings-name' }, '启用插件'),
            h('div', { className: 'dtt-settings-desc' },
              '关闭后悬浮窗隐藏、停止记录新线索；已有线索数据保留，可随时在此重新启用。')),
          h('button', {
        ...
        h('div', { className: 'dtt-settings-row' },
          h('div', { className: 'dtt-settings-text' },
            h('div', { className: 'dtt-settings-name' }, '跟随对话切换'),
            h('div', { className: 'dtt-settings-desc' },
              '切换对话时，任务线索自动跟随到当前对话：此前在工作区级 → 切到该对话所属工作区；此前在对话级 → 切到该对话。')),
          h('button', {
            ...
            'aria-checked': follow,
            disabled: saving,
            title: follow ? '已开启，点击关闭' : '已关闭，点击开启',
            onClick: function () { update({ followSession: !follow }); },
          }, h('span', { className: 'dtt-switch-knob' })),
        ),
        h('div', { className: 'dtt-settings-row' },
          h('div', { className: 'dtt-settings-text' },
            h('div', { className: 'dtt-settings-name' }, '从修改中学习'),
            h('div', { className: 'dtt-settings-desc' },
              '记录你手动合并/整理线索的操作，AI 后续总结时参考你的划分偏好与命名风格，越用越贴合你的习惯。')),
          h('button', {
            ...
            'aria-checked': learn,
            disabled: saving,
            title: learn ? '已开启，点击关闭' : '已关闭，点击开启',
            onClick: function () { update({ learnFromModifications: !learn }); },
          }, h('span', { className: 'dtt-switch-knob' })),
        ),
        h('div', { className: 'dtt-settings-row' },
          h('div', { className: 'dtt-settings-text' },
            h('div', { className: 'dtt-settings-name' }, '工作区视图模式'),
            h('div', { className: 'dtt-settings-desc' },
              '相似合并：跨会话的相似线索聚成一组，悬停展开组内线索；按对话分类：每个对话一张卡，悬停展开该对话的线索。')),
        ...
        h('div', { className: 'dtt-settings-row' },
          h('div', { className: 'dtt-settings-text' },
            h('div', { className: 'dtt-settings-name' }, '总结方式'),
            h('div', { className: 'dtt-settings-desc' },
              cfg
                ? (cfg.summarize === 'auto' ? '自动：LLM 优先，失败回退规则'
                  : cfg.summarize === 'llm' ? '仅 LLM 总结' : '仅规则模式（不调用模型）')
                : '加载中…'))),
        h('div', { className: 'dtt-settings-row' },
          h('div', { className: 'dtt-settings-text' },
            h('div', { className: 'dtt-settings-name' }, '刷新间隔'),
            h('div', { className: 'dtt-settings-desc' }, cfg ? (cfg.pollMs + ' ms') : '加载中…'))),
        h('div', { className: 'dtt-settings-foot' },
          '数据持久化于 DSH_HOME/data/topic-trail/；总结走 dsh 已配置的 DeepSeek 路由（DeepSeek API）。'),
      );
    }
      ...
      render() {
        ...
        if (this.state.err) {
          ...
          return h('div', {
            ...
            h('button', {
              style: {
                marginTop: 10, padding: '6px 14px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.2)',
    ...
    var appConfig = { followSession: true };
        ...
        try {
          disposeSettings = scope.slots.inject('settings.section', function () {
            return scope.slots.register(
              { name: 'settings.section', id: 'dsh-topic-trail', order: 60, label: function () { return '任务线索'; } },
              TopicTrailSettings
            );
          });
        } catch (e) { /* 设置槽位不可用不致命 */ }

...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ...
      '/* 瞳孔（数字） */',
      '.dtt-fab-pupil{position:relative;z-index:2;color:var(--dtt-ink);font-weight:700;font-size:13px;',
      ...
      /* 主动展示气泡 */
      '.dtt-bubble{position:fixed;z-index:10003;max-width:200px;padding:8px 12px;background:rgba(38,38,41,0.96);backdrop-filter:blur(16px);',
      '  border:1px solid rgba(255,255,255,0.12);border-radius:10px;color:#e5e7eb;font-size:11px;line-height:1.5;',
      ...
      '.dtt-bubble-topic{margin-top:6px;padding-top:5px;border-top:1px dashed rgba(255,255,255,0.14);',
      '  font-size:10px;color:#a8c2ff;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;display:flex;gap:4px;align-items:center}',
    ...
    function TopicTrailPanel() {
      ...
      var [bubble, setBubble] = useState(null); // 主动展示气泡 {text, x, y, hiding}
      ...
      // 手动逼泡用：暴露 effect 内部的 showBubble（连点小球三次时调用）
      var showBubbleRef = useRef(null);
      // 连点计数：{ n, at }。两次点击间隔超过 1.5 秒就重新计数
      var ballClicksRef = useRef({ n: 0, at: 0 });
      ...
      useEffect(function () {
        ...
        function fetchQuotes() {
          ...
          try {
            ...
            for (var i = 0; i < ts.length; i++) {
              if (ts[i] && ts[i].topics) {
                for (var j = 0; j < ts[i].topics.length; j++) {
                  topics.push({ title: ts[i].topics[j].title, status: ts[i].topics[j].status });
                }
              }
            }
          } catch (e) {}
          ...
          fetch('/plugins/topic-trail/bubble-quotes', {
            ...
            body: JSON.stringify({ skin: appConfig.skin, topics: topics.slice(0, 10) }),
        ...
        function showBubble(force) {
          ...
          function render() {
            var quotes = bubbleQuotesCache.current.length > 0 ? bubbleQuotesCache.current : pool;
            var quote = quotes.pop();
            bubbleQuotesCache.current = quotes;
            ...
            setBubble({ text: text, topic: relatedTopic, x: bx, y: by, hiding: false });
            hideTimer = setTimeout(function () {
              setBubble(function (b) { return b ? Object.assign({}, b, { hiding: true }) : null; });
              setTimeout(function () { setBubble(null); }, 400);
            }, 5000);
            timer = setTimeout(showBubble, 8000 + Math.random() * 10000);
          }
          // force（连点小球手动触发）时无视「面板已展开」，用户明知故点就该给反应
          if (!collapsed && !force) {
        ...
        timer = setTimeout(showBubble, 2000);
        return function () {
      ...
      }, [appConfig.proactiveDisplay, appConfig.skin, collapsed]);
      // 面板展开时立即隐藏当前气泡
      useEffect(function () {
        if (!collapsed && bubble) setBubble(null);
      }, [collapsed]);
      useEffect(function () {
      ...
      /** 连点小球计数：1.5 秒内累计到 3 次就返回 true 并清零（间隔超过 1.5 秒则重新计数）。 */
      function ballClickReached() {
        var now = Date.now();
        var st = ballClicksRef.current;
        if (now - st.at > 1500) st.n = 0;
        st.at = now;
        st.n += 1;
        if (st.n >= 3) { st.n = 0; return true; }
        return false;
      }
      ...
      var fabEl = h('button', {
        ...
        onClick: function () {
          ...
          // 先判定连点（要在切换面板状态之前算，第 3 次点击同时触发气泡）
          var wantBubble = ballClickReached();
          toggleCollapsed();
          if (wantBubble) forceBubble();
        },
      },
      ...
      var head = h('div', { className: 'dtt-head', onPointerDown: onPointerDown, style: { padding: (12.5 * scale) + 'px 12px', paddingLeft: (50.5 * scale) + 'px' } },
        h('span', {
          ...
          onClick: function (e) {
            e.stopPropagation();
            // 与悬浮小球共享同一套连点计数：在面板头部连点三次也会强制冒泡
            var wantBubble = ballClickReached();
            setCollapsed(true);
            if (wantBubble) forceBubble();
          },
      ...
      return [
        ...
        menu
          ...
          : null,
        bubble ? h('div', {
          className: 'dtt-bubble' + (bubble.hiding ? ' hiding' : '') + (appConfig.skin === 'deepseek' ? ' deepseek' : ''),
          style: { left: bubble.x + 'px', top: bubble.y + 'px', transform: 'translateX(-50%) translateY(-100%)', pointerEvents: bubble.topic ? 'auto' : 'none', cursor: bubble.topic ? 'pointer' : 'default' },
          onClick: function () {
            if (!bubble.topic) return;
            // 根据关联线索标题找到对应对话并跳转
            var targetSessionId = null;
            for (var i = 0; i < trails.length; i++) {
              var ts = trails[i].topics || [];
              for (var j = 0; j < ts.length; j++) {
                if (ts[j].title && ts[j].title.indexOf(bubble.topic) >= 0 || bubble.topic.indexOf(ts[j].title) >= 0) {
          ...
          h('div', null, bubble.text),
          // 关联线索：以前这个字段只用于判断可点击，从没显示过
          bubble.topic
            ? h('div', { className: 'dtt-bubble-topic', title: bubble.topic },
                h('em', null, t('bubble.topicLabel')), h('span', null, bubble.topic))
            : null,
        ) : null,

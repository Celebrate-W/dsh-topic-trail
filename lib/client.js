...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ...
      '.dtt-step{display:flex;align-items:flex-start;gap:8px;padding:8px 10px;min-height:38px;border-radius:8px;border-left:2px solid transparent;cursor:pointer;',
      '  transition:background 0.12s ease}',
      '.dtt-step:hover{background:rgba(255,255,255,0.05)}',
      '.dtt-step-jumping{background:rgba(103,158,254,0.12);border-left-color:#679efe;opacity:0.7;pointer-events:none}',
      ...
      '.dtt-step-main{flex:1;min-width:0}',
      '.dtt-step-title{font-size:12px;word-break:break-all;color:#e6e8ea;line-height:1.5}',
      '.dtt-step-detail{color:#9ca3af;font-size:11px;margin-top:1px;word-break:break-all;line-height:1.55;',
      ...
      '.dtt-fab{position:fixed;z-index:9999;pointer-events:auto;cursor:pointer;width:50px;height:50px;border-radius:50%;',
      '  display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:700;',
      '  background:linear-gradient(135deg,#3b82f6,#1d4ed8);color:#ffffff;',
      '  box-shadow:0 10px 28px rgba(29,78,216,0.5),0 2px 8px rgba(0,0,0,0.35);',
      '  border:1px solid rgba(255,255,255,0.18);transition:transform 0.15s ease,box-shadow 0.15s ease}',
      '.dtt-fab:hover{transform:scale(1.07);box-shadow:0 14px 36px rgba(29,78,216,0.6)}',
      '.dtt-fab:active{transform:scale(1.02)}',
    ...
    function TopicTrailPanel() {
      ...
      useEffect(function () {
        ...
        refresh();
        // 自适应轮询：页面隐藏时完全不请求；回到前台立即刷新一次；可见时每 5s 拉一次
        var pollMs = 5000;
      ...
      if (collapsed) {
        // 球上的数字 = 当前选中视图的线索数（与打开面板看到的一致，避免「球 99 / 面板 1」的困惑）
        var viewCount = 0;
        if (selectedWsId === '__all__') {
        ...
        if (fabX !== undefined) {
          fabX = Math.max(8, Math.min(fabX, (window.innerWidth || 0) - 58));
          fabY = Math.max(8, Math.min(fabY, (window.innerHeight || 0) - 58));
        }
        return h('button', {
          className: 'dtt-fab',
          style: {
      ...
      if (error) {
        ...
        body = topics.length === 0
          ? h('div', { className: 'dtt-empty' },
              h('span', { className: 'dtt-empty-ic' }, '◌'),
              '这个层级还没有可合并的工作线索\n先在某个会话里干点活，这里会自动汇总')
          : topics.map(function (t) { return renderMergedTopic(t, mergedHintFor(t)); });

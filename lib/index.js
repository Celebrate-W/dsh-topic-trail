...
const SUMMARIZE_SYSTEM_PROMPT = [
  ...
  '7. 如果输入末尾有「用户修改历史」段落，这些记录反映了用户的话题划分偏好（如用户习惯把哪些类型的话题合并、偏好的命名风格），请参考此逻辑划分新话题，但不要复述历史。',
].join('\n')
...
async function summarizeWithLlm(ctx, trail, config) {
  ...
  const lockedTopics = Array.isArray(trail.topics) ? trail.topics.filter((t) => t.locked) : []
  // 输入：时间线记录 + locked 话题清单（用户手动整理，必须保留）+ 修改历史（如开关开启）
  ...
  if (records.length === 0 && lockedTopics.length === 0) return []
  ...
  // 合并：locked 话题原样保留（用原始版本，不信任 LLM 对 locked 的改写），LLM 输出的非 locked 话题追加
  const lockedIds = new Set(lockedTopics.map((t) => t.id))
  const nonLockedLlm = llmTopics.filter((t) => !lockedIds.has(t.id))
...
export function apply(ctx, config) {
  ...
  let runtimeCfg = {}
  void loadRuntimeConfig().then((c) => { runtimeCfg = c }).catch(() => {})

...
const FOLLOW_UP_RE = /^(好的|可以|继续|嗯|好|行|ok|yes|对|对的对的|是啊|没错|谢谢|感谢|收到|没问题|试|试试|开始|来吧|整|搞|然后呢|然后|继续做|接着|详细点|具体点|展开|给个方案|再想|还有呢|别的|其他|换个|改一下|修正|优化|再说说)[。！!？?，,.\s]*$/i
/** 文本块类型里值得记录的块。 */
...
const SUMMARIZE_SYSTEM_PROMPT = [
  ...
  '请把它提炼成「话题（线索）」结构：话题 = 一条独立的工作意图/任务线（如"修复登录接口报错"、"写季度周报"、"调研竞品"）。一段会话可能有多条线索交叉推进。',
  ...
  '2. 话题 title 用口语化短语；summary 概括这条线索的目标。',
  '3. status：当前正在推进的线索标 active（通常只有 0-1 条），已完成或被搁置的标 done。',
  ...
  '   - 只有当记录中出现了全新的工作方向、与所有已有话题都不相关时，才创建新话题。',
...
function renderRecords(trail) {
  const fromBuffer = trail.buffer.map((rec) => {
    switch (rec.type) {
      case 'user':
        return `[seq=${rec.seq}] user: ${rec.text}`
      case 'assistant':
        return `[seq=${rec.seq}] assistant: ${rec.text}`
      case 'reasoning':
        return `[seq=${rec.seq}] reasoning: ${rec.text}`
      case 'tool':
        return `[seq=${rec.seq}] tool: ${rec.toolName}(${rec.args})`
      default:
        return `[seq=${rec.seq}] ${rec.type}`
    }
  })
  if (fromBuffer.length > 0) return fromBuffer
  const lines = []
  for (const topic of trail.topics) {
    if (topic.locked) continue // locked 话题（用户手动整理/合并）不参与重建，由 summarizeWithLlm 单独注入并要求保留
    for (const step of topic.steps) {
      if (step.kind === 'user') lines.push(`[seq=${step.seq}] user: ${step.detail}`)
      else if (step.kind === 'assistant') lines.push(`[seq=${step.seq}] assistant: ${step.detail}`)
      else if (step.kind === 'reasoning') lines.push(`[seq=${step.seq}] reasoning: ${step.detail}`)
      else if (step.kind === 'tool') lines.push(`[seq=${step.seq}] tool: ${step.toolName}(${step.detail})`)
    }
  }
  return lines
}
...
function appendRuleStep(trail, input) {
  ...
  if (isUser) {
    const followUp = typeof input.text === 'string' && FOLLOW_UP_RE.test(input.text.trim())
    if (!topic) {
      // 尚无任何线索 → 开新话题
...
async function summarizeWithLlm(ctx, trail, config) {
  const { provider, model } = await resolveLlmTarget(ctx, config)
  const records = renderRecords(trail)
  // preserveTopics 模式：所有已有话题都保留（不只 locked），LLM 只追加新话题
  const preserveAll = config.preserveTopics === true
  const lockedTopics = Array.isArray(trail.topics)
    ? trail.topics.filter((t) => t.locked || preserveAll)
    : []
  ...
  if (assembler.finish.kind === 'error' || assembler.finish.kind === 'aborted') {
    ...
    throw new Error(`llm stream ${assembler.finish.kind} (provider=${provider}, model=${model}) ${detail}`)
  }
  ...
  // 合并：locked 话题原样保留；LLM 输出的话题与已有话题标题相同则视为同一话题（不重复添加）
  const lockedIds = new Set(lockedTopics.map((t) => t.id))
  // 用所有已有话题（包括 locked）做标题匹配，避免重复
  const existingTitleToId = new Map()
  for (const t of trail.topics || []) {
    if (t && t.title) existingTitleToId.set(String(t.title).trim().toLowerCase(), t.id)
  }
  const nonLockedLlm = llmTopics
    .filter((t) => !lockedIds.has(t.id))
    .map((t) => {
      const key = String(t.title || '').trim().toLowerCase()
      if (key && existingTitleToId.has(key)) {
        return { ...t, id: existingTitleToId.get(key), updatedAt: new Date().toISOString(), _matched: true }
      }
      return t
    })
    // 标题匹配到已有话题的不重复添加（已在 lockedTopics 或原话题中）
    .filter((t) => !t._matched)
  return [...lockedTopics, ...nonLockedLlm]
}

/** 清洗 LLM 输出为内部 Topic 结构，id 基于 seq 保持稳定。 */
export function cleanLlmTopics(rawTopics) {
  ...
  for (const raw of rawTopics) {
    ...
    if (cleanedSteps.length === 0) continue
    const time = nowIso()
    topics.push({
      id: `t${cleanedSteps[0].seq}`,
...
function replayTrailFromEvents(trail, events, ctx, cfg) {
  trail.topics = []
  trail.buffer = []
  ...
  trail.buffer = sampleEvents(events, trail.maxBuffer)
...
export function apply(ctx, config) {
   ...
   */
  async function summarizeScope(scope, logger) {
    let targets = []
    ...
    void (async () => {
      ...
      if (imported.length > 0) {
        try { logger?.info?.(`[dsh-topic-trail] workspace summarize imported ${imported.length} sessions`) } catch { /* ignore */ }
      }
      let idx = 0
      const worker = async () => {
    ...
    return { ok: true, targets: targets.length, imported: 0, sessions: targets.map((t) => t.sessionId) }
  }
    ...
    handler: async (req, res) => {
      try {
        ...
        if (workspaceId === '__all__') {
          const result = await summarizeScope('all', ctx.logger)
          sendJson(res, 200, result)
          return
        }
        if (workspaceId) {
          const result = await summarizeScope('ws:' + workspaceId, ctx.logger)
          sendJson(res, 200, result)
          return
        }
        if (Array.isArray(sessionIds) && sessionIds.length > 0) {
          const result = await summarizeScope(sessionIds, ctx.logger)
          sendJson(res, 200, result)
          return
        }
        if (sessionId) {
          ...
          await summarizeTrail(ctx, trail, cfg, ctx.logger)
          sendJson(res, 200, { ok: true, topics: trail.topics })
          return
        }

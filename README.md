...
- **三级线索网络**：
  ...
  - 工作区和 dsh 界面保持一致（从 `dsh.workspace.view.v5` 读取分组，区分同路径下的不同 profile）；工作区可 📌 置顶；对话按最新操作时间降序排列。
...
- **双击线索跳转**：工作区/全部视图下双击合并线索，直接跳转到第一个来源会话。
- **步骤列表滚动**：话题步骤多时可滚动查看，不撑破面板。
...
- **仿 dsh 深色 UI**：毛玻璃面板、圆角卡片、用户/助手/工具/推理四色步骤侧条，导航弹层靠近边缘时自动反向展开，和 dsh 原生风格一致。
